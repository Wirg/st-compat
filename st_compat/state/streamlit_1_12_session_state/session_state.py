...
from .type_util import ValueFieldName, is_array_value_field_name
...
SCRIPT_RUN_WITHOUT_ERRORS_KEY: Final = f"{STREAMLIT_INTERNAL_KEY_PREFIX}_SCRIPT_RUN_WITHOUT_ERRORS"
...
class SessionState:
    ...
    def _call_callbacks(self) -> None:
        ...
        for wid in changed_widget_ids:
            try:
                ...
                st.warning("Calling st.experimental_rerun() within a callback is a no-op.")
    ...
    def _cull_nonexistent(self, widget_ids: Set[str]) -> None:
        ...
        self._old_state = {k: v for k, v in self._old_state.items() if (k in widget_ids or not _is_widget_id(k))}
...
def require_valid_user_key(key: str) -> None:
    ...
    if _is_widget_id(key):
        raise StreamlitAPIException(f"Keys beginning with {GENERATED_WIDGET_KEY_PREFIX} are reserved.")
