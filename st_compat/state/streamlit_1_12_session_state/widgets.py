...
from .type_util import ValueFieldName
...
def coalesce_widget_states(old_states: WidgetStates, new_states: WidgetStates) -> WidgetStates:
    ...
    for old_state in old_states.widgets:
        if old_state.WhichOneof("value") == "trigger_value" and old_state.trigger_value:
            ...
            if new_trigger_val and new_trigger_val.WhichOneof("value") == "trigger_value":
...
def _build_duplicate_widget_message(widget_func_name: str, user_key: Optional[str] = None) -> str:
    if user_key is not None:
...
def _get_widget_id(element_type: str, element_proto: WidgetProto, user_key: Optional[str] = None) -> str:
